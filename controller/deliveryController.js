...
const completeDelivery = async (req, res) => {
  try {
    ...
    console.log(`🚀 DELIVERY COMPLETE: Processing order ${order.invoice} (${order._id})`);
    ...
    if (order.cart && order.cart.length > 0) {
      console.log(`📦 DELIVERY: CALLING handleProductQuantity with ${order.cart.length} items`);
      ...
      console.log(`✅ DELIVERY: handleProductQuantity completed for order ${order.invoice}`);
    } else {
      console.warn(`⚠️ DELIVERY: No cart items found for order ${order.invoice}`);
    }
