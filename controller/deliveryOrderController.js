...
const markAsDelivered = async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(`🚀 DELIVERY ORDER COMPLETE: Processing order ${order.invoice} (${order._id})`);
      console.log(`📋 Order cart:`, order.cart ? order.cart.length : 'No cart');
      
      // Reduce product stock
      if (order.cart && order.cart.length > 0) {
        console.log(`📦 DELIVERY ORDER: CALLING handleProductQuantity with ${order.cart.length} items`);
        ...
        console.log(`✅ DELIVERY ORDER: handleProductQuantity completed for order ${order.invoice}`);
      } else {
        console.warn(`⚠️ DELIVERY ORDER: No cart items found for order ${order.invoice}`);
      }
