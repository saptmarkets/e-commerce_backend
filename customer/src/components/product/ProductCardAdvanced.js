...
const ProductCardAdvanced = ({ product, attributes, className = "" }) => {
  ...
  const handleAddToCart = () => {
    ...
    if (!isUnitAvailable) {
      ...
      if (product.stock === 0) {
        ...
        return notifyError(`Insufficient stock for ${selectedUnit.unitTitle || selectedUnit.unit?.name}. Need ${packQty} pieces but only ${product.stock} available. Try smaller units.`);
      }
      return notifyError("This unit is not available!");
    }
    
    if (quantity > availableStock) {
      return notifyError(`Only ${availableStock} ${selectedUnit.unitTitle || selectedUnit.unit?.name} available!`);
    }
    ...
    const cartItem = {
      ...
      unitTitle: selectedUnit.title || `${selectedUnit.packQty || 1} ${selectedUnit.unit?.name || 'pcs'}`,
    };
  ...
  const getStockMessage = () => {
    ...
    if (availableStock === 0) {
      const packQty = selectedUnit.packQty || 1;
      return `Insufficient stock for ${selectedUnit.unitTitle || selectedUnit.unit?.name}. Need ${packQty} pieces but only ${product.stock} available. Try smaller units.`;
    }
    return `${availableStock} ${selectedUnit.unitTitle || selectedUnit.unit?.name} available`;
  };
              ...
              >
                <div className="flex flex-col">
                  <span className="font-medium">
                    {selectedUnit?.title || `${selectedUnit?.packQty || 1} ${selectedUnit?.unit?.name || 'pcs'}`}
                  </span>
                  {pricingInfo && (
                    <span className="text-sm text-gray-500">
                      ${getNumberTwo(pricingInfo.perUnitPrice)} per {selectedUnit?.unit?.name || 'piece'}
                    </span>
                  )}
                </div>
                        ...
                        <div className="flex justify-between items-start">
                          <div className="flex flex-col">
                            <span className={`font-medium ${!isUnitStockAvailable ? 'text-gray-400' : ''}`}>
                              {unit.title || `${unit.packQty || 1} ${unit.unit?.name || 'pcs'}`}
                            </span>
                            <span className={`text-sm ${!isUnitStockAvailable ? 'text-gray-400' : 'text-gray-500'}`}>
                              ${getNumberTwo(perUnitPrice)} per {unit.unit?.name || 'piece'}
                            </span>
