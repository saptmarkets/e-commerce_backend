...
const SpecialOfferCard = ({ product, attributes, promotion }) => {
  ...
  const handleAddItem = () => {
    ...
    notifySuccess(`Added ${quantityToAdd} ${pricing?.unit || 'items'} to cart!`);
  };
  ...
  return (
    <>
      ...
      <div className="relative bg-white rounded-xl overflow-hidden shadow-lg transition-all duration-300 hover:shadow-xl border border-red-100 max-w-sm mx-auto">
        ...
        >
          <Image
            src={product.image && product.image.length > 0 ? product.image[0] : '/images/placeholder.png'}
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-right">
                <div className="text-xs text-gray-600 font-medium">
                  Min: {pricing.minQty} {pricing.unit}
                </div>
                {pricing.maxQty && (
                  <div className="text-xs text-gray-500">
                    Max: {pricing.maxQty} {pricing.unit}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-red-50 rounded-lg p-2 border border-red-100">
              <div className="text-center">
                <div className="text-xs text-red-600 font-medium mb-1">
                  Get {pricing.minQty} {pricing.unit} for
                </div>
                <div className="text-xl font-bold text-red-700">
                  ${getNumberTwo(pricing.promotionalPrice)}
                </div>
                <div className="text-xs text-red-500">
                  Instead of ${getNumberTwo(pricing.originalPrice)}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {product.stock <= 0 ? (
              ...
              >
                ...
                Add {pricing.minQty} {pricing.unit} to Cart
              </button>
            )}
            ...
            <div className="text-xs text-center text-gray-500">
              {pricing.minQty > 1 && (
                <span>Minimum order: {pricing.minQty} {pricing.unit}</span>
              )}
              {pricing.maxQty && (
                <span className="ml-2">• Max on offer: {pricing.maxQty} {pricing.unit}</span>
              )}
            </div>
          </div>
        </div>
