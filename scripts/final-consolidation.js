...
async function finalConsolidation() {
	...
	for (const cat of allCategories) {
		...
		if (!normalized) continue; // Skip categories with no name
	...
	for (const [key, cats] of groups.entries()) {
		...
		// Sort by: product count (desc), child count (desc), creation date (asc)
		scored.sort((a,b)=>{
...
finalConsolidation().catch(err=>{ console.error('❌ Error consolidating:', err); process.exit(1); }); 