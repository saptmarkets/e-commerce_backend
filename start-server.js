...
const corsOptions = {
  ...
  allowedHeaders: ['Content-Type', 'Authorization', 'company', 'Origin', 'X-Requested-With', 'Accept', 'idempotency-key', 'if-match'],
  credentials: true,
...
app.use((req, res, next) => {
  ...
  res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept, Authorization, company, idempotency-key, if-match');
  }
