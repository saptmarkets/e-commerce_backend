...
const SalesAnalytics = () => {
  ...
  // 🎸 PDF Document Component - Elvis-approved styling!
  const PDFDocument = ({ data, title, timestamp }) => (
    <Document>
      <Page size="A4" style={pdfStyles.page}>
        <View style={pdfStyles.header}>
          <Text style={pdfStyles.title}>SaptMarkets Sales Analytics Report</Text>
          <Text style={pdfStyles.subtitle}>{title}</Text>
          <Text style={pdfStyles.date}>Generated: {timestamp}</Text>
        </View>
        
        <View style={pdfStyles.content}>
          {data.overview && (
            <View style={pdfStyles.section}>
              <Text style={pdfStyles.sectionTitle}>📊 Overview & KPIs</Text>
              {data.overview.map((item, index) => (
                <View key={index} style={pdfStyles.row}>
                  <Text style={pdfStyles.cell}>{Object.keys(item)[0]}: {Object.values(item)[0]}</Text>
                </View>
              ))}
            </View>
          )}
          ...
          {data.trends && data.trends.length > 0 && (
            <View style={pdfStyles.section}>
              <Text style={pdfStyles.sectionTitle}>📈 Sales Trends</Text>
              {data.trends.slice(0, 10).map((item, index) => (
                <View key={index} style={pdfStyles.row}>
                  <Text style={pdfStyles.cell}>{item.Date} - {item.Revenue}</Text>
                </View>
              ))}
            </View>
          )}
          ...
          {data.paymentMethods && data.paymentMethods.length > 0 && (
            <View style={pdfStyles.section}>
              <Text style={pdfStyles.sectionTitle}>💳 Payment Methods</Text>
              {data.paymentMethods.map((item, index) => (
                <View key={index} style={pdfStyles.row}>
                  <Text style={pdfStyles.cell}>{item['Payment Method']}: {item.Revenue}</Text>
                </View>
              ))}
            </View>
          )}
          ...
          {data.topProducts && data.topProducts.length > 0 && (
            <View style={pdfStyles.section}>
              <Text style={pdfStyles.sectionTitle}>🏆 Top Products</Text>
              {data.topProducts.slice(0, 10).map((item, index) => (
                <View key={index} style={pdfStyles.row}>
                  <Text style={pdfStyles.cell}>{item['Product Name']}: {item.Revenue}</Text>
                </View>
              ))}
            </View>
          )}
          ...
          {data.categories && data.categories.length > 0 && (
            <View style={pdfStyles.section}>
              <Text style={pdfStyles.sectionTitle}>📦 Category Sales</Text>
              {data.categories.map((item, index) => (
                <View key={index} style={pdfStyles.row}>
                  <Text style={pdfStyles.cell}>{item['Category Name']}: {item.Revenue}</Text>
                </View>
              ))}
            </View>
          )}
        ...
        <View style={pdfStyles.footer}>
          <Text style={pdfStyles.footerText}>🎸 Generated by SaptMarkets Analytics System</Text>
          <Text style={pdfStyles.footerText}>Elvis says: "Thank you, thank you very much!"</Text>
        </View>
      </Page>
    </Document>
  );

  // 🎸 PDF Styles - Making it look professional!
  ...
  const exportToPDF = async (data, filename) => {
    try {
      ...
      const doc = <PDFDocument data={data} title={filename} timestamp={timestamp} />;
      const asPdf = pdf(doc);
      ...
      alert('PDF export failed. Please try again or use CSV format. 🎸');
    }
  };
            ...
            <div className="space-y-6">
              ...
              <div className="bg-purple-50 dark:bg-purple-900 p-4 rounded-lg">
                ...
                <p className="text-sm text-purple-700 dark:text-purple-300 italic">
                  "Export that data, baby! CSV, Excel, and PDF - all rockin' and ready to go!" 🎵
                </p>
              </div>
            </div>
          </div>
