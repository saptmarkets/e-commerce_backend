...
const SidebarSubMenu = ({ route }) => {
  ...
  const isActiveGroup = route.routes.some(r => location.pathname.startsWith(r.path));
  const [open, setOpen] = useState(isActiveGroup);

  useEffect(() => {
    console.log("SidebarSubMenu route:", route.name, "routes:", route.routes);
  }, [route]);
  ...
  return (
    <>
      <li className="relative px-6 py-3" key={route.name}>
        <button
          className={`inline-flex items-center justify-between focus:outline-none w-full text-sm font-semibold transition-colors duration-150 ${isActiveGroup ? 'text-emerald-700 dark:text-gray-100' : 'hover:text-emerald-600 dark:hover:text-gray-200'}`}
        ...
        {open && (
          <ul
            className="p-2 ml-2 overflow-hidden text-sm font-medium text-gray-500 rounded-md dark:text-gray-400 dark:bg-gray-900 border-l-2 border-gray-100 dark:border-gray-700"
            aria-label="submenu"
          >
