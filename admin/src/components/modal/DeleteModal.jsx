...
const DeleteModal = ({ id, ids, setIsCheck, category, title, useParamId, handleDelete: customHandleDelete, isOpen, handleModalClose }) => {
  ...
  // Use custom modal state if provided, otherwise use context
  const modalOpen = isOpen !== undefined ? isOpen : isModalOpen;
  const handleCloseModal = handleModalClose || closeModal;
  ...
  const handleDelete = async () => {
    ...
    // If a custom handleDelete function is provided, use it
    if (customHandleDelete) {
      try {
        setIsSubmitting(true);
        await customHandleDelete();
        handleCloseModal();
      } catch (error) {
        console.error('Custom delete error:', error);
        notifyError(error.message || 'Delete operation failed');
      } finally {
